...
description : "This is the first chapter in the Introduction to R course. You will learn how to use the console as a calculator and how to assign variables. You will also get to know the basic data types in R. Let's get started!"
attachments : 
  slides_link: http://s3.amazonaws.com/assets.datacamp.com/production/course_1203/slides/R_environment.pdf


--- type:MultipleChoiceExercise xp:50 skills:1 key:0378d3d2a2
## What is R?

*R* and *Python* are the two most popular programming languages for statistics, data science, and econometrics.  *R* has many important features that make it very attractive to users.  Which of the following statements about *R* are *not* true?  

1. *R* is an open-source software environment for statistical computing and graphics,
2. *R* users must submit requests to a committee of experts to obtain new features, 
3. *R* can be very slow compared to *C++*,
4. *R* is an interpreted language, allowing interactive code development,
5. *R* cannot call software written in *C++*,
6. *R* is a functional language where every operator is an *R* function,
7. *R* supports object-oriented programming with classes and methods,
8. *R* cannot perform *for* loops,
9. *R* is extended through user-created packages, providing for the latest developments, such as *Machine Learning*,
10. *R* has metaprogramming facilities that allow programming on the language,


*** =instructions
- statements (3) and (8) are false; the others are true.
- statements (4) and (6) are false; the others are true.
- statements (2), (5) and (8) are false; the others are true.
- statements (3) and (7) are false; the others are true.

*** =hint
Read about R in the attached slides.
...
*** =sct
```{r}
msg1 = "R can be extended through user-created packages."
msg2 = "R is written in C++, and can easily call C++ code."
msg3 = "R can perform for loops, but they are slow."
msg4 = "R supports object-oriented programming."
...
*** =instructions
Add a line with the comment text *Calculate 11 - 5*, in the script window on the upper right, just above the *R* code that calculates *11 - 5*.

*** =hint
Simply add the line `# Calculate 11 - 5` above the *R* code that calculates 11 - 5.
...
test_output_contains("6", incorrect_msg = "Do not remove the code that calculates 11 - 5.")
test_student_typed(c("# Calculate 11 - 5", "# calculate 11 - 5", "#Calculate 11 - 5", "#calculate 11 - 5",
                     "# Calculate 11-5", "# calculate 11-5", "#Calculate 11-5", "#calculate 11-5"), 
                   not_typed_msg = "Make sure to add the comment: `# Calculate 11 - 5`")
...
There are things that make *R* the immensely popular language that it is today. On the other hand, there are also aspects of *R* that are less attractive. Which of the following statements are true regarding this statistical programming language developed by Ihaka and Gentleman in the nineties?
